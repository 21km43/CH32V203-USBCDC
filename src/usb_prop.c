...
void USBSerial_config(uint32_t baud, uint8_t stop_bit, uint8_t parity_bit, uint8_t data_bit, uint8_t timeout)
{
  cdc_comconfig[0] = (uint8_t)(baud);
  cdc_comconfig[1] = (uint8_t)(baud >> 8);
  cdc_comconfig[2] = (uint8_t)(baud >> 16);
  cdc_comconfig[3] = (uint8_t)(baud >> 24);
  cdc_comconfig[4] = stop_bit;
  cdc_comconfig[5] = parity_bit;
  cdc_comconfig[6] = data_bit;
  cdc_comconfig[7] = timeout;
}
